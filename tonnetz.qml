    ...
    Keys.onPressed: event => {
                        ...
                        if (canvas.hasSelNode) {
                            ...
                            canvas.scrollIntoView(
                                canvas.triadCenter(ti, tj,
                                                   canvas.selTriadMajor))
                            ...
                            tonnetzController.selectTriad(root, third, fifth,
                                                          canvas.selTriadMajor)
    ...
    Canvas {
        ...
        Connections {
            ...
            function onSelectionChanged() {
                ...
                if (t === 1) {
                    if (canvas.hasSelNode && canvas.selNodeI === ni
                            && canvas.selNodeJ === nj)
                        return
                    ...
                    if (canvas.hasSelTriad && canvas.selTriadI === ni
                            && canvas.selTriadJ === nj
        ...
        Connections {
            ...
            function onNrDistancesEnabledChanged() {
                if (!tonnetzController.nrDistancesEnabled) {
                    ...
                    var root = canvas.selTriadMajor ? canvas.noteAt(
                                                          canvas.selTriadI,
                                                          canvas.selTriadJ) : canvas.noteAt(
                                                          canvas.selTriadI,
                                                          canvas.selTriadJ + 1)
                    canvas.nrDists = tonnetzController.computeTriadDistances(
                                root, canvas.selTriadMajor)
                }
                canvas.requestPaint()
            }
        }
        ...
        function hitTest(px, py) {
            ...
            for (var di = -1; di <= 1; di++) {
                for (var dj = -1; dj <= 1; dj++) {
                    ...
                    var dist = Math.sqrt(
                                (px - np.x) * (px - np.x) + (py - np.y) * (py - np.y))
                    if (dist <= r) {
                        return {
                            "type": "note",
                            "i": ci + di,
        ...
        onPaint: {
            ...
            if (activeNotes !== 0) {
                ...
                for (var i = iMin; i <= iMax; i++) {
                    for (var j = jMin; j <= jMax; j++) {
                        ...
                        if (isAct(noteAt(i, j)) && isAct(noteAt(i + 1, j))
                                && isAct(noteAt(i, j + 1))) {
                            var tp0 = nodePos(i,
                                              j), tp1 = nodePos(
                                                      i + 1,
                        ...
                        if (isAct(noteAt(i + 1, j))
                                && isAct(noteAt(i, j + 1)) && isAct(
                                    noteAt(i + 1, j + 1))) {
                            var tp0 = nodePos(i + 1,
                                              j), tp1 = nodePos(
                                                      i,
                ...
                for (var ti = iMin; ti <= iMax; ti++) {
                    for (var tj = jMin; tj <= jMax; tj++) {
                        ...
                        if (majActive)
                            ...
                            ctx.fillText(chordLabel(ti, tj, true),
                                         cMaj.x, cMaj.y)
                        ...
                        if (minActive)
                            ...
                            ctx.fillText(chordLabel(ti, tj, false),
                                         cMin.x, cMin.y)
                    }
                }
            }
            ...
            for (var ni = iMin; ni <= iMax; ni++) {
                for (var nj = jMin; nj <= jMax; nj++) {
                    ...
                    if (np.x < -r || np.x > width + r || np.y < -r
                            || np.y > height + r)
                        continue
                    var isActive = (hasSelNode && ni === selNodeI
                                    && nj === selNodeJ) || isAct(noteAt(ni, nj))
            ...
            if (activeLabels.length > 0) {
                ...
                for (var al = 0; al < activeLabels.length; al++)
                    ctx.fillText(activeLabels[al].text, activeLabels[al].x,
                                 activeLabels[al].y)
            }
        }
        ...
        function drawActiveEdgeOverlay(ctx, rng, fn, color) {
            ...
            for (var i = rng.iMin; i <= rng.iMax; i++) {
                for (var j = rng.jMin; j <= rng.jMax; j++) {
                    ...
                    if (i + 1 <= rng.iMax && j - 1 >= rng.jMin && fn(
                                noteAt(i + 1, j - 1)))
        ...
        function isActiveTriad(i, j, isMajor) {
            if (isMajor)
                return (hasSelTriad && i === selTriadI && j === selTriadJ
                        && selTriadMajor) || (isAct(noteAt(i, j)) && isAct(
                                                  noteAt(i + 1, j))
                                              && isAct(noteAt(i, j + 1)))
            else
                return (hasSelTriad && i === selTriadI && j === selTriadJ
                        && !selTriadMajor) || (isAct(noteAt(i + 1, j))
                                               && isAct(noteAt(
                                                            i, j + 1)) && isAct(
                                                   noteAt(i + 1, j + 1)))
        }
            ...
            onReleased: mouse => {
                            ...
                            if (hit.type === "note") {
                                if (canvas.hasSelNode
                                    ...
                                    canvas.selNodeJ = hit.j
                                    canvas.nrDists = []
                                    tonnetzController.selectNote(hit.semitone,
                                                                 hit.i, hit.j)
                                    visualizerSwitcher.setNoteSelection(hit.i,
                                                                        hit.j)
                                }
                            } else {
                                if (canvas.hasSelTriad
                                    ...
                                    tonnetzController.selectTriad(hit.root,
                                                                  hit.third,
                                                                  hit.fifth,
                                                                  hit.isMajor)
                                    visualizerSwitcher.setTriadSelection(
