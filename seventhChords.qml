...
Item {
	...
	Canvas {
		...
		readonly property int effectiveDepth: showDistances ? 4 : selDepth
		...
		onPaint: {
			...
			function edgeAlpha(a, b) {
				if (!hasSel || effectiveDepth === 0)
					return 0.1
				if (a === selNode || b === selNode)
					return 1.0
				if (effectiveDepth >= 2 && nodeDists.length > 0) {
					var da = nodeDists[a], db = nodeDists[b]
					if (da >= 1 && db >= 1 && Math.max(da,
													   db) <= effectiveDepth)
						return 0.4
				}
				return 0.1
			...
			for (var ni = 0; ni < nodes.length; ni++) {
				...
				} else if (nrD >= 1 && nrD <= effectiveDepth) {
