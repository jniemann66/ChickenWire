...
Item {
    ...
    Keys.onPressed: event => {
                        if (event.key === Qt.Key_F5) {
                            ...
                            if (canvas.showDistances && canvas.selNode >= 0) {
                                canvas.nodeDists = canvas.computeDistances(
                                    canvas.selNode)
                            } else {
                                canvas.nodeDists = []
                            }
    ...
    Canvas {
        ...
        function nodePos(idx) {
            var n = nodes[idx]
            return Qt.point(originX + n.lx * baseUnit * scale,
        ...
        // Edge table
        // [nodeA, nodeB, type]
        property var edges: [// Db+ hexatonic cycle  (F –L– Am –P– A –L– C#m –P– Db –L– Fm –P– F)
            [4, 5, "L"], [5, 7, "P"], [7, 9, "L"], [9, 8, "P"], [8, 6, "L"], [6, 4, "P"], // C+ hexatonic cycle   (C –P– Cm –L– Ab –P– Abm –L– E –P– Em –L– C)
            [10, 11, "P"], [11, 13, "L"], [13, 15, "P"], [15, 14, "L"], [14, 12, "P"], [12, 10, "L"], // D+ hexatonic cycle   (Dm –L– Bb –P– Bbm –L– F# –P– F#m –L– D –P– Dm)
            [16, 17, "L"], [17, 19, "P"], [19, 21, "L"], [21, 20, "P"], [20, 18, "L"], [18, 16, "P"], // Eb+ hexatonic cycle  (Ebm –L– B –P– Bm –L– G –P– Gm –L– Eb –P– Ebm)
            [22, 23, "L"], [23, 25, "P"], [25, 27, "L"], [27, 26, "P"], [26, 24, "L"], [24, 22, "P"], // C+  augmented → its 3 major triads (own cycle) + 3 minor triads (Db+ cycle)
            [0, 10, "aug"], [0, 13, "aug"], [0, 14, "aug"], [0, 5, "aug"], [0, 6, "aug"], [0, 9, "aug"], // Db+ augmented → its 3 major triads (own cycle) + 3 minor triads (D+ cycle)
            [1, 4, "aug"], [1, 7, "aug"], [1, 8, "aug"], [1, 16, "aug"], [1, 19, "aug"], [1, 20, "aug"], // Eb+ augmented → its 3 major triads (own cycle) + 3 minor triads (C+ cycle)
            [2, 23, "aug"], [2, 24, "aug"], [2, 27, "aug"], [2, 11, "aug"], [2, 12, "aug"], [2, 15, "aug"], // D+  augmented → its 3 major triads (own cycle) + 3 minor triads (Eb+ cycle)
        ...
        property var cubeEdges: [// Aug-twin links (faint cross-cluster connectors)
            [28, 29, "augLink"] // C+
            , [30, 31, "augLink"] // Db+
            , [32, 33, "augLink"] // Eb+
            , [34, 35, "augLink"] // D+
            , // C+ in Db+ cluster (28) → 3 minors of that cluster: Am, Fm, C#m
            [28, 5, "aug"], [28, 6, "aug"], [28, 9, "aug"], // C+ in C+ cluster  (29) → 3 majors of that cluster: C, Ab, E
            [29, 10, "aug"], [29, 13, "aug"], [29, 14, "aug"], // Db+ in Db+ cluster (30) → 3 majors: F, A, Db
            [30, 4, "aug"], [30, 7, "aug"], [30, 8, "aug"], // Db+ in D+ cluster  (31) → 3 minors: Dm, Bbm, F#m
            [31, 16, "aug"], [31, 19, "aug"], [31, 20, "aug"], // Eb+ in C+ cluster  (32) → 3 minors: Cm, Em, Abm
            [32, 11, "aug"], [32, 12, "aug"], [32, 15, "aug"], // Eb+ in Eb+ cluster (33) → 3 majors: B, Eb, G
            [33, 23, "aug"], [33, 24, "aug"], [33, 27, "aug"], // D+ in D+ cluster   (34) → 3 majors: Bb, D, F#
            [34, 17, "aug"], [34, 18, "aug"], [34, 21, "aug"], // D+ in Eb+ cluster  (35) → 3 minors: Ebm, Bm, Gm
        ...
        onPaint: {
            ...
            var augHW = Math.max(6, 17 * scale)
            // augmented square half-width
            ...
            for (var ni = 0; ni < nodes.length; ni++) {
                ...
                if (n.type === "aug") {
                    ...
                    if (sel || augGlow) {
                        ...
                        var nrClrA = nrC[nrD]
                        ctx.fillStyle = Qt.rgba(nrClrA.r, nrClrA.g,
                                                nrClrA.b, 0.35)
                    ...
                    ctx.fillStyle = (sel
                                     || augGlow) ? Theme.selText : (nrD >= 0
                                                                    && nrD < nrC.length) ? nrC[nrD] : Theme.nodeText
                    ...
                    if (r > 6) {
                        ...
                        ctx.fillStyle = (active
                                         || sel) ? Theme.hlColor : (nrD >= 0
                                                                    && nrD < nrC.length) ? nrC[nrD] : Theme.triadText
                        ctx.fillText(nodeLabel(ni), np.x, np.y)
                    }
                }
            }
        ...
        MouseArea {
            ...
            onReleased: mouse => {
                            ...
                            if (hit < 0) {
                                ...
                                if (n.type !== "aug")
                                tonnetzController.selectTriad(
                                    n.s0, n.s1, n.s2, n.type === "major")
