    ...
    Canvas {
        ...
        Connections {
            ...
            function onSelectionChanged() {
                ...
                if (t === 1) {
                    if (canvas.hasSelNote && canvas.selNoteI === ni && canvas.selNoteJ === nj)
                        return;
                    ...
                    if (canvas.hasSel && canvas.selI === ni && canvas.selJ === nj && canvas.selMajor === maj)
                        return;
                    canvas.hasSel = true;
                    canvas.selI = ni;
                    canvas.selJ = nj;
        ...
        function dualPos(I, J, isMajor) {
            ...
            return Qt.point(originX + (I + off) * dx + (J + off) * dx / 2, originY - (J + off) * dy);
        }
        ...
        function isTriadActive(i, j, isMaj) {
            if (hasSel && i === selI && j === selJ && isMaj === selMajor)
                return true;
            return isMaj ? (isAct(noteAt(i, j)) && isAct(noteAt(i, j + 1)) && isAct(noteAt(i + 1, j))) : (isAct(noteAt(i, j + 1)) && isAct(noteAt(i + 1, j)) && isAct(noteAt(i + 1, j + 1)));
        }
            ...
            if (r > 9) {
                ...
                for (var fi = iMin; fi <= iMax; fi++) {
                    for (var fj = jMin; fj <= jMax; fj++) {
                        ...
                        var noteActive = (hasSelNote && fi === selNoteI && fj === selNoteJ) || isAct(s);
                        if (augmented) {
                            ctx.beginPath();
        ...
        MouseArea {
            ...
            onReleased: mouse => {
                ...
                if (!hit)
                    return;
                if (hit.type === "triad") {
                    if (canvas.hasSel && canvas.selI === hit.i && canvas.selJ === hit.j && canvas.selMajor === hit.isMajor) {
                        // toggle off
                        canvas.hasSel = false;
                        canvas.nrDists = [];
