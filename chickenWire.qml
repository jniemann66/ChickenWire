...
Item {
    ...
    Keys.onPressed: event => {
                        if (event.key === Qt.Key_F5) {
                            tonnetzController.nrDistancesEnabled
                            = !tonnetzController.nrDistancesEnabled
                            event.accepted = true
                            return
                        }
                        if (!canvas.hasSel && !canvas.hasSelNote)
                        return
                        var di = 0, dj = 0
                        ...
                        if (canvas.hasSelNote) {
                            ...
                            tonnetzController.selectNote(canvas.noteAt(ni, nj),
                                                         ni, nj)
                            canvas.nrDists = []
                            ...
                            canvas.selI = ni
                            canvas.selJ = nj
                            canvas.scrollIntoView(canvas.dualPos(
                                                      ni, nj, canvas.selMajor))
                            var t = canvas.triadNotes(ni, nj, canvas.selMajor)
                            tonnetzController.selectTriad(t.root,
                                                          t.third, t.fifth,
                                                          canvas.selMajor)
                            canvas.nrDists = tonnetzController.nrDistancesEnabled ? tonnetzController.computeTriadDistances(t.root, canvas.selMajor) : []
                            visualizerSwitcher.setTriadSelection(
                                ni, nj, canvas.selMajor)
                        }
    ...
    Canvas {
        ...
        function isAug(s) {
            return showAugmented && isAct(s) && isAct((s + 4) % 12) && isAct(
                        (s + 8) % 12)
        }
        ...
        Connections {
            ...
            function onSelectionChanged() {
                ...
                if (t === 1) {
                    if (canvas.hasSelNote && canvas.selNoteI === ni
                            && canvas.selNoteJ === nj)
                        return
                    ...
                    if (canvas.hasSel && canvas.selI === ni
                            && canvas.selJ === nj && canvas.selMajor === maj)
                        return
                    canvas.hasSel = true
                    canvas.selI = ni
                    canvas.selJ = nj
        ...
        Connections {
            ...
            function onNrDistancesEnabledChanged() {
                if (!tonnetzController.nrDistancesEnabled) {
                    ...
                    var t = canvas.triadNotes(canvas.selI, canvas.selJ,
                                              canvas.selMajor)
                    canvas.nrDists = tonnetzController.computeTriadDistances(
                                t.root, canvas.selMajor)
                }
                canvas.requestPaint()
            }
        }
        ...
        function dualPos(I, J, isMajor) {
            ...
            return Qt.point(originX + (I + off) * dx + (J + off) * dx / 2,
                            originY - (J + off) * dy)
        }
        ...
        function visibleRange() {
            ...
            var corners = [Qt.point(0, 0), Qt.point(width, 0), Qt.point(
                               0, height), Qt.point(width, height)]
            for (var c = 0; c < 4; c++) {
        ...
        function isTriadActive(i, j, isMaj) {
            if (hasSel && i === selI && j === selJ && isMaj === selMajor)
                return true
            return isMaj ? (isAct(noteAt(i, j)) && isAct(noteAt(i, j + 1))
                            && isAct(noteAt(i + 1,
                                            j))) : (isAct(noteAt(i, j + 1)) && isAct(noteAt(i + 1, j)) && isAct(noteAt(i + 1, j + 1)))
        }
        ...
        onPaint: {
            ...
            ctx.clearRect(
                        0, 0, width,
                        height) // clear to transparent; background is in Main.qml
                ...
                for (var fi = iMin; fi <= iMax; fi++) {
                    for (var fj = jMin; fj <= jMax; fj++) {
                        if (isAct(noteAt(fi, fj)))
                            drawHexFace(ctx, fi, fj, Theme.hlFaceFill,
                                        Theme.hlHexStroke)
                        if (hasSelNote && fi === selNoteI && fj === selNoteJ)
                            drawHexFace(ctx, fi, fj, Theme.selHexFill,
                                        Theme.selHexStroke)
                    }
                }
            }
            ...
            for (var i = iMin; i <= iMax; i++) {
                for (var j = jMin; j <= jMax; j++) {
                    ...
                    drawEdge(ctx, pm, dualPos(i, j, false),
                             Theme.edgeL) // L — blue
                    drawEdge(ctx, pm, dualPos(i - 1, j, false),
                             Theme.edgeR) // R — orange
                    drawEdge(ctx, pm, dualPos(i, j - 1, false),
                             Theme.edgeP) // P — pink
                }
            }
            ...
            if (r > 9) {
                ...
                for (var fi = iMin; fi <= iMax; fi++) {
                    for (var fj = jMin; fj <= jMax; fj++) {
                        ...
                        var noteActive = (hasSelNote && fi === selNoteI
                                          && fj === selNoteJ) || isAct(s)
                        if (augmented) {
                            ctx.beginPath()
                        ...
                        if (noteActive)
                            activeNoteLabels.push({
                                                      "text": label,
                                                      "x": hc.x,
                                                      "y": hc.y
                                                  })
                        else
                            ctx.fillText(label, hc.x, hc.y)
                    }
                }
            }
            ...
            for (var ni = iMin; ni <= iMax; ni++) {
                for (var nj = jMin; nj <= jMax; nj++) {
                    for (var nm = 0; nm < 2; nm++) {
                        ...
                        if (np.x < -r || np.x > width + r || np.y < -r
                                || np.y > height + r)
                            continue
                        var active = isTriadActive(ni, nj, isMaj)
                        var nrRt = rootNote(ni, nj, isMaj)
                        var nrD = nrDists.length
                                > 0 ? (isMaj ? nrDists[nrRt] : nrDists[nrRt + 12]) : -1
                        ...
                        if (active) {
                            ...
                            var nrClr = [Theme.nrDist0, Theme.nrDist1, Theme.nrDist2, Theme.nrDist3, Theme.nrDist4, Theme.nrDist5, Theme.nrDist6][nrD]
                            ctx.fillStyle = Qt.rgba(nrClr.r, nrClr.g,
                                                    nrClr.b, 0.35)
            ...
            if (activeNoteLabels.length > 0) {
                ...
                for (var al = 0; al < activeNoteLabels.length; al++)
                    ctx.fillText(activeNoteLabels[al].text,
                                 activeNoteLabels[al].x, activeNoteLabels[al].y)
            }
        }
        ...
        MouseArea {
            ...
            onReleased: mouse => {
                            parent.parent.forceActiveFocus(
                                ) // keep keyboard focus on root Item
                            if (didDrag)
                            return
                            ...
                            if (hit.type === "triad") {
                                if (canvas.hasSel && canvas.selI === hit.i
                                    ...
                                    canvas.selMajor = hit.isMajor
                                    canvas.hasSelNote = false
                                    var t = canvas.triadNotes(hit.i, hit.j,
                                                              hit.isMajor)
                                    tonnetzController.selectTriad(t.root,
                                                                  t.third,
                                                                  t.fifth,
                                                                  hit.isMajor)
                                    canvas.nrDists = tonnetzController.nrDistancesEnabled ? tonnetzController.computeTriadDistances(t.root, hit.isMajor) : []
                                    visualizerSwitcher.setTriadSelection(
                                        hit.i, hit.j, hit.isMajor)
                                }
                            } else {
                                if (canvas.hasSelNote
                                    ...
                                    canvas.hasSel = false
                                    canvas.nrDists = []
                                    tonnetzController.selectNote(
                                        canvas.noteAt(hit.i, hit.j),
                                        hit.i, hit.j)
                                    visualizerSwitcher.setNoteSelection(hit.i,
                                                                        hit.j)
                                }
                            }
                            canvas.requestPaint()
